...
def save_checkpoint(save_path, net_state, epoch, filename='checkpoint.pth.tar'):
    ...
    # f.write('save checkpoint to ' + filename + '\n')
    # f.close()
    pass
...
def filter_points_wy(points, warped_points, shape, return_mask=False):
    ### check!
    warped_points = warped_points.float()
    shape = shape.float()
    mask = (warped_points >= 0) * (warped_points <= shape-1)
    mask = (torch.prod(mask, dim=-1) == 1)
    if return_mask:
        return points[mask].clone(), warped_points[mask].clone(), mask
    return points[mask].clone(), warped_points[mask].clone()
    # return points [torch.prod(mask, dim=-1) == 1]
...
def inv_warp_image(img, mat_homo_inv, device='cpu', mode='bilinear'):
    ...
    # # plot test image
    # test_img = img[:, :, 0]
    # test_img_array = (test_img.detach().cpu().numpy() * 255.0).astype(int)
...
def descriptor_loss(descriptors, descriptors_warped, homographies, mask_valid=None, 
    ...
    # config
    from utils.utils_color import warp_points
    lamda_d = lamda_d # 250
    margin_pos = 1
    margin_neg = 0.2
...
def extract_pnts(labels_2D, homography):
    batch_pnts = []
    batch_warped_pnts = []
    ...
    for i in range(B):
        ...
        pnts, warped_pnts = warpPoints(pnts, H, W, homography[i])
...
def warpPoints(pnts, H, W, homography):
    if isinstance(pnts, torch.Tensor):
        pnts = pnts.long()
    else:
        pnts = torch.tensor(pnts).long()
    warped_pnts = warp_points(torch.stack((pnts[:, 0], pnts[:, 1]), dim=1),
                              homography_scaling_torch(homography, H, W))
