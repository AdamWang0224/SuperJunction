...
class Train_model_frontend(object):
    ...
    def loadModel(self):
        ...
        # # freeze part of the layers

        # # below is to freeze detector network in SuperPointNet_gauss2
        # for para in net.parameters():
        #     para.requires_grad = False
        # for para in net.convDa.parameters():
        #     para.requires_grad = True
        # for para in net.bnDa.parameters():
        #     para.requires_grad = True
        # for para in net.convDb.parameters():
        #     para.requires_grad = True
        # for para in net.bnDb.parameters():
        #     para.requires_grad = True

        # # below is to freeze detector network in SuperPointNet
        # for para in net.parameters():
        #     para.requires_grad = False
        # for para in net.convDa.parameters():
        #     para.requires_grad = True
        ...
        # # below is to freeze descriptor network in SuperPointNet
        # for para in net.parameters():
        #     para.requires_grad = False
        # for para in net.convPa.parameters():
        #     para.requires_grad = True
        # for para in net.convPb.parameters():
        #     para.requires_grad = True

        # # # below is to freeze descriptor head in SuperPointNet
        # for para in net.parameters():
        #     para.requires_grad = True
        # for para in net.convDa.parameters():
        #     para.requires_grad = False
    ...
    def warpPoints(self, pnts, H, W, homography):
        from utils.utils import homography_scaling_torch as homography_scaling
        from utils.utils import filter_points_wy
        ...
        pnts, warped_pnts = filter_points_wy(pnts, warped_pnts, torch.tensor([W, H]))

        return pnts, warped_pnts

    def extract_pnts(self, labels_2D, homography):
        batch_pnts = []
        batch_warped_pnts = []
        B, _, H, W = labels_2D.size()
        for i in range(B):
            # convert 2D labels to keypoint positions in (x, y)
            temp = labels_2D[i, 0, :, :]
            pnts = torch.nonzero(temp)

            ## change the order of x and y
            # indices = torch.tensor([1, 0])
            # pnts = torch.index_select(pnts, 1, indices)

            # compute warped points
            pnts, warped_pnts = self.warpPoints(pnts, H, W, homography[i])

            batch_pnts.append(pnts)
            batch_warped_pnts.append(warped_pnts)

        return batch_pnts, batch_warped_pnts
    ...
    def train(self, **options):
        ...
        n_epoch = 1 # 5, interval for checkpoint/validation, 100 for oct dataset
        max_epoch = 300 # 300, 100000
        while epoch <= max_epoch:
            ...
            for i, sample_train in tqdm(enumerate(self.train_loader)):
                ...
                pnts, warped_pnts = self.extract_pnts(sample_train["labels_2D"], sample_train['homographies'])
                # train one sample
                loss_out, detector_loss, descriptor_loss, vessel_loss = self.train_val_sample(sample_train, pnts, warped_pnts,
                                                                                              epoch, self.n_iter, True)
                self.n_iter += 1
                if epoch % n_epoch == 0:
            ...
            if epoch % n_epoch == 0:
                ...
                #             break
            # run validation
            if self._eval and epoch % n_epoch == 0:
                logging.info("====== Validating...")
                running_losses = []
                running_losses_detector = []
                running_loss_descriptor = []
                ...
                for j, sample_val in enumerate(self.val_loader):
                    # extract keypoints and warped keypints
                    pnts, warped_pnts = self.extract_pnts(sample_val["labels_2D"], sample_val['homographies'])
                    loss_out, detector_loss, descriptor_loss, vessel_loss = self.train_val_sample(sample_val,  pnts, warped_pnts,
                                                                                                  epoch, self.n_iter + j, False)
                    running_losses.append(loss_out)
                    running_losses_detector.append(detector_loss)
                    running_loss_descriptor.append(descriptor_loss)
                    running_loss_vessel.append(vessel_loss)
                ...
                #     self.saveModel()
            if epoch % n_epoch == 0:
                logging.info(f"save model: every {n_epoch} epoch, current epoch: {epoch}")
                self.saveModel(epoch)

            # ending condition
            if epoch > max_epoch:
                # end training
                logging.info("End training: %d", epoch)
                break
            else:
                epoch += 1
    ...
    def printLosses(self, losses, task="train"):
        ...
        """
        # f = open('training_output.txt', 'a+')
        print(f"{task} loss: {losses['loss'].item()}")
        # for element in list(losses):
        #     # print ('add to tb: ', element)
        #     print(task, "-", element, ": ", losses[element].item())

        #     f.write(str(task) + "-" + str(element) + ": " +
        #             str(losses[element].item()) + '\n')
        # f.write('\n')
        # f.close()
